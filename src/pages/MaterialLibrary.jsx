...
function CustomMaterialCard({ item, onDelete, onRefresh }) {
  ...
  return (<div className="bg-surface border border-primary/30 rounded-xl overflow-hidden">
    <div className="aspect-[1.6] bg-surface-container-low flex items-center justify-center relative group">{img ? <img src={img} alt="" className="w-full h-full object-contain" /> : <Icon name="add_circle" className="text-on-surface-variant text-3xl opacity-30" />}{edit && (<><label className="absolute bottom-2 right-2 bg-primary text-white p-1.5 rounded-full cursor-pointer shadow"><Icon name="add_photo_alternate" className="text-sm" /><input type="file" accept="image/*" className="hidden" onChange={e=>{if(e.target.files[0])handleImage(e.target.files[0]);e.target.value=''}} /></label>{img && <button onClick={()=>setImg(null)} className="absolute top-2 right-2 bg-white/90 p-1 rounded-full shadow"><Icon name="close" className="text-xs text-error" /></button>}</>)}</div>
    <div className="p-3">
