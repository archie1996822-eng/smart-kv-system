...
import { peripheralChecklist, compressImage, extractColors, startNanoDraw, pollNanoResult, visionModels, generateModels } from '../data/stitchApi';
...
export default function Workbench() {
  ...
  const handleImageSet=async(img)=>{
    ...
    try{const r=await extractColors(img.dataUrl);setAnalysis(r);setStatusMsg('分析完成')}catch(err){setStatusMsg('分析失败: '+err.message)}
    setProcessing(false);
  };
    ...
    for(let i=0;i<tg.length;i++){
      ...
      try{
        ...
        if (result && result._direct) {
          ...
          // Nano Banana: poll for results
          setStatusMsg(`${item.name}: 等待生成...`);
          const imgs = await pollNanoResult(result);
          imgUrl = imgs[0]?.url;
        }
        localResults[item.id] = {status:'done',imageUrl:imgUrl,title:item.name};
  ...
  return (<Layout><div className="pt-6 pb-12 px-8 max-w-[1440px] mx-auto">
    ...
    <div className="flex flex-col items-center justify-center my-6">
      <button onClick={generateAll} disabled={!kvImage||!analysis||generating||processing||selected.length===0}
